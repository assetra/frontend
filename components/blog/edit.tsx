...
const Edit: React.FC<EditItemProps> = ({ id }) => {
  ...
  const fetchBlog = async () => {
    try {
      ...
      console.log(blog, selectedAuthor, title, selectedTags, content);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://gtxadmin.pythonanywhere.com/edit_blogs/${id}`,
        {
          method: "PUT",
          headers: {
