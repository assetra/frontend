...
import PortfolioIcon from "../icons/portfolio/PortfolioIcon";
...
const MenuBar = ({ active, settingOpen, setSettingOpen }: IMenuBarProps) => {
  ...
  const handleClick = (page: number) => {
    if (page === 1) router.push("/mobile/portfolio");
    if (page === 2) router.push("/mobile/coinMarket");
  ...
  return (
    <div className="control-wrapper bg-[#0e0f18] pt-2 flex items-center justify-evenly h-[110px] fixed left-0 bottom-0 w-full">
      <PortfolioIcon isActive={active === 1} onClick={() => handleClick(1)} />
      <TransactionListIcon
        isActive={active === 2}
