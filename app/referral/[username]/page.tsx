...
export default function Page({ params }: { params: { username: string } }) {
  const referrer = params.username;
    ...
    if (emailRegex.test(email)) {
      try {
        ...
        if (response.ok) {
          setEmail("");
          setFeedbackMessage(
            "Referral recorded. Please proceed with the sign-up."
          );
          setIsSuccess(true);
          setUser(result.user);
          setTimeout(() => {
  ...
  return (
    <>
      <div className="grid grid-cols-2 min-h-[100svh]">
        <div className={`bg-black text-white pb-8 px-6 pt-32`}>
        ...
        <div className="bg-white text-black text-center px-6 pb-8 pt-36">
          <h1 className="text-xl text-[2rem]">Welcome to GTX!</h1>
          <div className="mt-2 text-sm text-[#6978A0]">
            Hello there! Please enter your email to proceed.
          </div>
          {feedbackMessage && (
            <div
